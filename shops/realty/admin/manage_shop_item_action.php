...
/**
 * KShops24 부동산 매물 관리 액션 처리 모듈 (manage_shop_item_action.php)
 * - 부동산 정책 수정, 매물 카테고리/매물 추가, 수정, 삭제 및 AJAX 상태 변경을 처리합니다.
 */
...
try {
    $pdo->exec("ALTER TABLE shop_items MODIFY COLUMN trade_type VARCHAR(50) NOT NULL DEFAULT '매매' COMMENT '거래 유형'");
} catch (Exception $e) {
    // 컬럼이 아예 없는 경우 새로 추가
    try {
        $pdo->exec("ALTER TABLE shop_items ADD COLUMN trade_type VARCHAR(50) NOT NULL DEFAULT '매매' COMMENT '거래 유형' AFTER cat_id");
    } catch (Exception $e2) {
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_shop']) && !isset($_POST['flyer_order'])) {
    ...
    // [다국어] 부동산 정책 번역 데이터 수집 및 JSON 변환
    $translations = [];
    ...
    // [신규] 커스텀 거래 유형을 ui_settings에 저장
    if (isset($_POST['custom_trade_types'])) {
        $stmt_ui = $pdo->prepare("SELECT ui_settings FROM shops WHERE id = ?");
        $stmt_ui->execute([$shop_id]);
        $current_ui = json_decode($stmt_ui->fetchColumn() ?: '{}', true);
        $current_ui['custom_trade_types'] = trim($_POST['custom_trade_types']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_item'])) {
    ...
    $trade_type = $_POST['trade_type'] ?? '매매';
    ...
    if (isset($_POST['ajax_update'])) {
        ...
        header('Content-Type: application/json');
        echo json_encode(['status' => 'success', 'message' => '신규 매물이 등록되었습니다.']);
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_item'])) {
    ...
    if (isset($_POST['ajax_update'])) {
        ...
        header('Content-Type: application/json');
        echo json_encode(['status' => 'success', 'message' => '매물 정보가 수정되었습니다.']);
        exit;
    }
