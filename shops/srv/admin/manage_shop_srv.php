...
/**
 * KShops24 서비스/예약 관리 모듈 (manage_shop_srv.php)
...
// 부동산 카테고리 설정 로드
$category_config_path = $_SERVER['DOCUMENT_ROOT'] . "/shops/srv/srv_config.php";
...
<div class="container-fluid p-0">
    ...
    if (isset($_GET['msg'])) {
        ...
        switch ($_GET['msg']) {
            ...
            case 'board_deleted':
                ...
                break;
            case 'item_added':
                $msg_text = '신규 서비스가 등록되었습니다.';
                break;
            case 'item_updated':
                $msg_text = '서비스 정보가 수정되었습니다.';
                break;
            case 'item_deleted':
                $msg_text = '서비스가 삭제되었습니다.';
                $msg_type = 'warning';
                break;
            case 'label_updated':
    ...
    <?php echo renderPageHeader('서비스 관리', 'bi-card-checklist'); ?>
        ...
        <div class="col-12">
            <div class="<?php echo UI_SECTION_CARD; ?>">
                <div class="p-3 p-md-4 d-flex flex-column h-100">
                    <?php echo renderSectionHeader('서비스 등록 현황', 'bi-bar-chart-line'); ?>
                    <div class="d-flex justify-content-between align-items-center mb-2"><span class="text-dark">총 등록 서비스</span><span class="fw-bold fs-5"><?php echo number_format($item_stats['total'] ?? 0); ?>개</span></div>
                    <div class="d-flex justify-content-between align-items-center mb-2"><span class="text-danger">예약 마감</span><span class="fw-bold text-danger"><?php echo number_format($item_stats['soldout'] ?? 0); ?>개</span></div>
        ...
        <div class="col-12">
            <div class="<?php echo UI_SECTION_CARD; ?>">
                <div class="p-3 p-md-4 d-flex flex-column h-100">
                    <?php 
                    /**
                     * 서비스 리스트 섹션 헤더 렌더링 (상수가 class 속성일 때)
                    ...
                    echo renderSectionHeader(
                        ...
                        'bi-grid-3x3-gap',
                        [
                            '<i ' . UI_INFO_SM_LABEL . '></i> 처음 서비스를 등록하는 경우, 반드시 <a href="manage_shop.php?pg=manage_shop_srv_policy#item_category">서비스 정책 관리에서 "전체 서비스의 서비스 카테고리 설정"</a>을 하셔야 합니다.',
                            '<i ' . UI_INFO_SM_LABEL . '></i> 마우스로 드래그하여 순서를 변경할 수 있습니다.'
                        ],
                        '<button class="btn btn-primary btn-sm rounded-pill px-3 ms-md-auto" onclick="openAddItemModal()"><i class="bi bi-plus-lg me-1"></i> 서비스 추가</button>'
                    ); 
                    ?>
                    ...
                    <div class="list-group list-group-flush border-top" id="item-list-sortable">
                        ...
                        <?php if (empty($grouped_items)): ?>
                            <div class="list-group-item text-center py-5 text-muted border-bottom">등록된 서비스가 없습니다.</div>
                        <?php else: ?>
                                        ...
                                        <div class="d-flex flex-column flex-md-row align-items-md-center gap-2 gap-md-3 w-100">
                                            <div class="d-flex align-items-start flex-grow-1" style="min-width: 0;">
                                                ...
                                                <div class="d-flex flex-column justify-content-center flex-grow-1" style="min-width: 0;">
                                                    <div class="mb-1 d-flex flex-wrap align-items-center gap-1">
                                                        <?php if ($m['is_hide']): ?><span class="badge bg-secondary">숨김</span><?php endif; ?>
                                                        <?php if ($m['is_soldout']): ?><span class="badge bg-danger">마감</span><?php endif; ?>
                                                    ...
                                                    <div class="fw-bold text-truncate mb-1" style="font-size: 0.95rem;">
                                                        <span style="cursor: pointer; text-decoration: underline; text-decoration-color: rgba(0, 74, 173, 0.4); text-underline-offset: 4px; position: relative; z-index: 10;" onclick="previewItemDetail(JSON.parse(this.closest('.sort-item-row').querySelector('.menu-list-img').getAttribute('data-item')), 'ko')" title="서비스 상세 미리보기">
                                            ...
                                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-end align-items-md-center flex-md-shrink-0 mt-2 mt-md-0 w-100 w-md-auto info-action-container">
                                                ...
                                                <div class="text-end ms-md-3 w-100 w-md-auto border-top border-md-0 pt-2 pt-md-0 d-flex justify-content-end gap-1">
                                                    ...
                                                    <a href="manage_shop.php?pg=manage_shop_srv&del_item=<?php echo $m['id']; ?>" class="btn btn-sm btn-outline-danger border-0" onclick="return confirm('삭제하시겠습니까?')"><i class="bi bi-trash"></i><span class="d-md-none ms-1">삭제</span></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<!-- --- [모달] 추가/수정 등 팝업 UI 분리 --- -->
<!-- --------------------------------------------------------- -->
<?php include_once __DIR__ . '/manage_shop_srv_modals.php'; ?>
...
<script>
    function calculateDiscount() {
        ...
        if (rate > 0) {
            discountPriceInput.value = Math.floor(price * (1 - rate / 100));
        } else {
    ...
    // [추가] 서비스 상세 미리보기 (다국어 지원 및 읽기 전용 모드)
    function previewItemDetail(item, langCode = 'ko') {
        ...
        if (discountRate > 0) badgesHtml += `<span class="badge bg-danger me-1">할인</span>`;
    ...
    function openAddItemModal() {
        ...
        if (titleEl) titleEl.innerText = "새 서비스 등록";
        ...
        if (youtubeContainer) {
            ...
            addYoutubeInput(); // 새 서비스 등록 시 빈 입력창 1개 추가
        }
    ...
    function openEditItemModal(item) {
        ...
        const titleEl = modalEl.querySelector('.modal-title');
        if (titleEl) titleEl.innerText = "서비스 정보 수정";
        ...
        setVal('item_price', item.item_price || "");
        ...
        if (itemListEl) {
            Sortable.create(itemListEl, {
                ...
                onEnd: async function(evt) {
                    ...
                    try {
                        const res = await fetch('manage_shop.php?pg=manage_shop_srv', {
                            method: 'POST',
                            body: formData,
                            headers: {
