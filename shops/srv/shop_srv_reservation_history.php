...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cancel') {
        ...
        try {
            // 보안: 본인의 문의 내역 중 취소 가능한 상태인 경우에만 취소 허용
            $stmt = $pdo->prepare("UPDATE shop_inquiries SET status = 'cancelled' WHERE id = ? AND shop_id = ? AND customer_phone = ? AND status IN ('pending', 'contacted', 'confirmed')");
            $stmt->execute([$inquiry_id, $shop_id, $phone]);
            if ($stmt->rowCount() > 0) {
                echo json_encode(['status' => 'success']);
            } else {
    ...
    // [추가] 상태 폴링 (poll) - 고객 브라우저에서 예약 상태 실시간 확인
    if ($action === 'poll') {
        header('Content-Type: application/json');
        try {
            $stmt = $pdo->prepare("SELECT id, status, customer_inquiry, inquiry_data, customer_phone, created_at, owner_reply FROM shop_inquiries WHERE shop_id = ? AND customer_phone = ? ORDER BY created_at DESC LIMIT 10");
            ...
            foreach ($inquiries as $inq) {
                $orders[] = [
                    'id' => $inq['id'],
                    'status' => $inq['status'],
                    'items' => $inq['inquiry_data'],
                    'total_price' => 0,
                    'order_no' => 'RES-'.$inq['id'],
                    'created_at' => $inq['created_at'],
                    'order_type' => 'reservation',
                    'owner_reply' => $inq['owner_reply']
                ];
            }
    ...
    try {
        ...
        $status_map = [
            'pending' => ['label' => '예약 대기', 'color' => 'bg-warning text-dark'],
            'contacted' => ['label' => '예약 확정', 'color' => 'bg-info text-white'],
            'confirmed' => ['label' => '예약 확정', 'color' => 'bg-info text-white'],
            'completed' => ['label' => '서비스 완료', 'color' => 'bg-success'],
            'cancelled' => ['label' => '예약/상담 취소', 'color' => 'bg-secondary']
        ];
        ...
        foreach ($inquiries as $inq) {
            ...
            $item_names = [];
            if (is_array($items)) foreach ($items as $item) $item_names[] = htmlspecialchars($item['name'] ?? '서비스');
            ...
            if ($context !== 'admin') {
                ...
                if (in_array($inq['status'], ['pending', 'contacted', 'confirmed'])) {
                    $html .= "<button type='button' class='btn btn-sm btn-outline-warning py-0 px-2 shadow-sm' style='font-size: 0.75rem; border-radius: 12px;' onclick='confirmCancelInquiry({$inq['id']})'>예약취소</button>";
                }
            ...
            $html .= "</div>
                        <h6 class='fw-bold text-dark mb-2 text-truncate'><i class='bi bi-card-checklist me-1'></i>{$item_str}</h6>
