...
/**
 * KShops24 서비스 개인정보처리방침 팝업 페이지 (privacy_policy.php)
 * 설명: 이메일 등의 외부 링크에서 "개인정보처리방침 자세히 보기"를 눌렀을 때, 
...
try {
    // 2. DB에서 개인정보처리방침 데이터만 추출
    $stmt = $pdo->prepare("SELECT set_value FROM site_settings WHERE set_key = 'privacy_policy'");
    $stmt->execute();
    $terms_of_use = $stmt->fetchColumn();

    if (!$terms_of_use) {
        $terms_of_use = "<div class='text-center text-muted py-5'>등록된 개인정보처리방침이 없습니다.</div>";
    } else {
...
<body>
    <div class="terms-card">
        <h4 class="fw-bold text-center" style="color: #004aad;">서비스 개인정보처리방침</h4>
